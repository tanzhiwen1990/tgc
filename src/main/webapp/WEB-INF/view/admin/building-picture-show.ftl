...
<div class="page-container">
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
	     <span class="l" style="margin-right:20px;"> 
			<input type="button" id="checkall" name="checkall" value="全选" style="width:80px;height:30px;border-radius:4px;background:#dd514c;color:#fff;font-size:14px; "/> 
		</span>
		<span class="l"> 
			<a href="javascript:;" onclick="deletePicture()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> 
		</span> 
		
		<span class="r">共有图片：<strong>${listSize}</strong> 张</span>
	</div>
		...
		<#if listSize != 0>
			...
			<li class="item">
				<div class="portfoliobox">
					<input class="checkbox" name="checkname" type="checkbox" value="${l.building_picture_id}">
					<div class="picbox"><a href="${basePath}${l.building_picture_addr}" data-lightbox="gallery" data-title="图片"><img src="${basePath}${l.building_picture_addr}"></a></div>
				</div>
			</li>
			</#list>
		<#else>
...
function deletePicture(){
	...
	layer.confirm('确认要删除一张或多张图片吗？',function(){
		$.ajax({ 
	    	type: "post", 
	    	url: "${basePath}" + "building/deletePicture", 
	    	data: {
	    		ids : ids
	    	}, 
	    	dataType: "json", 
...
$(function(){
	...
	var uploader = WebUploader.create({
		...
		// 文件接收服务端。
		server: '${basePath}building/BuildingUp_Files',
