...
<div class="page-container">
	<form class="form form-horizontal" id="myform" action="${ctx}Property/edit?id=${obj.id}" method="Post">
		<div class="row cl">
			<label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span>物业类型：</label>
			<div class="formControls col-xs-8 col-sm-9">
				<input type="text" class="input-text" value="${obj.type}" placeholder="物业类型" id="" dataType="s" name="type" style="width:200px;">
			</div>
		</div>
	...
	<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
				<input id="d11" type="text" onClick="WdatePicker({skin:'whyGreen',dateFmt: 'yyyy-MM-dd HH:mm:ss',  maxDate: 'new Date()' })" name="time" value="${obj.time?string('yyyy-MM-dd hh:mm:ss')}" class="input-text Wdate" style="width:200px;"/>
			</div>
	</div>
	-->
		...
		<div class="row cl">
			...
			<div class="formControls col-xs-8 col-sm-9">
			   <textarea name="description" id="description" rows="5" cols="80"  dataType="s" value="${obj.description}">${obj.description}</textarea> 
			</div>
		</div>
	   ...
	   <#if obj.state=='已上线'>
		...
		<div class="row cl">
			<div class="col-xs-8 col-sm-9 col-xs-offset-4 col-sm-offset-2">
				...
				<button onClick="reset();" class="btn btn-default radius" type="button">&nbsp;&nbsp;取消&nbsp;&nbsp;</button>
			</div>
		</div>
	</form>
</div>
