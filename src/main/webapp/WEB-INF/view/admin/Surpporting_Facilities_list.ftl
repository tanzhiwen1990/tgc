...
<div class="page-container">
	<div class="text-c"> <span class="select-box inline">
		...
		<input type="text" name="" id="name" placeholder=" 设备名称" style="width:250px" class="input-text">
		<button name="search" id="search" class="btn btn-success" type="submit"><i class="Hui-iconfont">&#xe665;</i> 搜索</button>
	</div>
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			<a href="javascript:;" onclick="application.batchDelete('${basePath}/surpporting_facilities/delete')" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> 
		</span> 
		<a class="btn btn-primary radius" onClick="application.locationToUrl('配套添加','${basePath}/surpporting_facilities/addPage');" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加</a>
		...
		<table class="table table-border table-bordered table-bg table-hover">
			...
			<#list page.data as ob>
				<tr class="text-c">
					...
					<td>${ob.id}</td>
					<td>${ob.name}</td>
				    <td><img src="${basePath}${ob.picUrl}" style="width:200px;height:100px;"/></td>
					<td>${ob.username}</td>
					...
					<td class="td-manage">
					<#if ob.state == '已上线'>
						<a style="text-decoration:none"  onclick="application.changeStatus(this,'未上线',${ob.id},'${basePath}/surpporting_facilities/changeStatus')"><i class="Hui-iconfont">&#xe6de;</i></a> 
					<#else>
						<a style="text-decoration:none"  onclick="application.changeStatus(this,'已上线',${ob.id},'${basePath}/surpporting_facilities/changeStatus')"><i class="Hui-iconfont">&#xe603;</i></a> 
					</#if>
						<a style="text-decoration:none" class="ml-5" onclick="application.locationToUrl('配套编辑','${basePath}/surpporting_facilities/editPage?id=${ob.id}')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> 
						<a style="text-decoration:none" class="ml-5" onclick="application.deleteOne(this,'${ob.id}','${basePath}/surpporting_facilities/delete')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
					</td>
...
<script type="text/javascript">
	...
	function ajaxPage(num){
		$.ajax({ 
	        type: "post", 
	        url: "${basePath}" + "surpporting_facilities/ajaxPage", 
	        data: {
	        	beginNum : num,
	...
	function change(result){
		...
		for(var i = 0 ; i<result.data.length ; i++){
		   ...
		   if(result.data[i].state=="已上线"){
		     ...
		     btn= "<a style='text-decoration:none' onClick=\"application.changeStatus(this,'未上线',"+result.data[i].id+",'${basePath}/surpporting_facilities/changeStatus')\" href='javascript:;' title='上线'><i class='Hui-iconfont'>&#xe6de;</i></a>";
            }else{
              btn= "<a style='text-decoration:none' onClick=\"application.changeStatus(this,'已上线',"+result.data[i].id+",'${basePath}/surpporting_facilities/changeStatus')\" href='javascript:;' title='下线'><i class='Hui-iconfont'>&#xe603;</i></a>";
              status='<td class="td-status"><span class="label label-defaunt radius">未上线</span></td>';
            }
			var d = new Date();
			d.setTime(result.data[i].time);
			img="<img src='${basePath}"+result.data[i].picUrl+"' />";
			$("#pagetbody").append("<tr class='text-c'>" +
					  ...
					  "<td class='td-manage'>" +
						  btn +
						  "<a style='text-decoration:none' class='ml-5' onclick=\"application.locationToUrl('配套编辑','${basePath}/surpporting_facilities/editPage?id="+result.data[i].id+"')\"  title='编辑'><i class='Hui-iconfont'>&#xe6df;</i></a> " +
						  "<a style='text-decoration:none' class='ml-5' onclick=\"application.deleteOne(this,"+result.data[i].id+",'${basePath}\/surpporting_facilities\/delete')\" href='javascript:;' title='删除'><i class='Hui-iconfont'>&#xe6e2;</i></a>" +
					  "</td></tr>");
				}
