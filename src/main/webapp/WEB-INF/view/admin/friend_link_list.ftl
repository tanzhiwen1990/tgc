...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>合作管理<span class="c-gray en">&gt;</span>友情链接列表<a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		<span class="l">
			<a href="javascript:;" onclick="application.batchDelete('${ctx}/friendLink/delete')" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> 
		</span> 
		<a class="btn btn-primary radius" onClick="application.locationToUrl('添加','${ctx}/friendLink/addPage')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加</a>
		...
		<table class="table table-border table-bordered table-bg table-hover">
			...
			<#list page.data as ob>
				<tr class="text-c">
					...
					<td>${ob.id}</td>
					<td>
					 <#if  ob.linkurl?contains("http://")>
						<a href="${ob.linkurl}"  title="点击查看">${ob.companyname}</a>
					  <#elseif ob.linkurl?contains("https://")>
					     <a href="${ob.linkurl}"  title="点击查看">${ob.companyname}</a>
				      <#else>
				        <a href="http://${ob.linkurl}"  title="点击查看">${ob.companyname}</a>
					  </#if>
				    <td>
				    <#if ob.logo??>
				        <img src="${ctx}${ob.logo}" />
					...
					<#--
					 <td>
					 <#if  ob.linkurl?contains("http://")>
						<a href="${ob.linkurl}" >${ob.linkurl}</a>
					  <#elseif ob.linkurl?contains("https://")>
					     <a href="${ob.linkurl}"  >${ob.linkurl}</a>
				      <#else>
				        <a href="http://${ob.linkurl}"  >${ob.linkurl}</a>
					  </#if>
					</td>
					-->
					
					<td>${ob.username}</td>
					<td>${ob.releaseDate?string('yyyy-MM-dd HH:mm:ss')}</td>
					<#if ob.status=="未上线">
				    ...
				    <td class="td-manage">
					<#if ob.status =='已上线'>
						<a style="text-decoration:none"  onclick="application.changeStatus(this,'未上线',${ob.id},'${ctx}/friendLink/changeStatus')"><i class="Hui-iconfont">&#xe6de;</i></a> 
					<#else>
						<a style="text-decoration:none"  onclick="application.changeStatus(this,'已上线',${ob.id},'${ctx}/friendLink/changeStatus')"><i class="Hui-iconfont">&#xe603;</i></a> 
					</#if>
						<a style="text-decoration:none" class="ml-5" onclick="application.locationToUrl('编辑','${ctx}/friendLink/editPage?id=${ob.id}')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> 
						<a style="text-decoration:none" class="ml-5" onclick="application.deleteOne(this,'${ob.id}','${ctx}/friendLink/delete')" href="javascript:;" title="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
					</td>
	...
	function change(result){
		...
		for(var i = 0 ; i<result.data.length ; i++){
		    ...
		    if(result.data[i].status=="已上线"){
             ...
             var d=new Date();
             d.setTime(result.data[i].releaseDate);
			 img="<img src='${ctx}"+result.data[i].logo+"'/>";
			$("#pagetbody").append("<tr class='text-c'>" +
			          "<td><input type='checkbox' class='taid' name='ids' value='"+result.data[i].id+"'/></td>" +
					  "<td>"+result.data[i].id +"</td>" +
					  "<td>"+result.data[i].companyname +"</td>" +
					  "<td>"+img +"</td>" +
					  "<td>"+result.data[i].description +"</td>" +
					  "<td>"+result.data[i].linkurl +"</td>" +
					  "<td>"+result.data[i].username +"</td>" +
					  "<td>"+d.Format("yyyy-MM-dd hh:mm:ss") +"</td>"+
					   status +
					  "<td class='td-manage'>" +
						  btn +
						  "<a style='text-decoration:none' class='ml-5' onclick=\"application.locationToUrl('编辑','${ctx}/friendLink/editPage?id="+result.data[i].id+"')\"  title='编辑'><i class='Hui-iconfont'>&#xe6df;</i></a> " +
						  "<a style='text-decoration:none' class='ml-5' onclick=\"application.deleteOne(this,"+result.data[i].id+",'${ctx}\/friendLink\/delete')\" href='javascript:;' title='删除'><i class='Hui-iconfont'>&#xe6e2;</i></a>" +
					  "</td></tr>");
