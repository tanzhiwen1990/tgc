...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 内容管理 <span class="c-gray en">&gt;</span> 购房知识列表 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
...
<div class="page-container">
	...
	<div class="cl pd-5 bg-1 bk-gray mt-20"> 
		...
		<a class="btn btn-primary radius" onClick="application.locationToUrl('购房知识添加','${ctx}/problem/addPage')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加</a>
		...
		<table class="table table-border table-bordered table-bg table-hover">
			...
			<#list page.data as ob>
				<tr class="text-c">
					...
					<td><a href="${ob.url}">${ob.id}</a></td>
					<td>${ob.title}</td>
					<td> ${ob.description}</td>
					<td style="width:200px;height:70px;"> <img src="${ctx}${ob.imgurl} " style="width:200px;height:70px;"></td>
					<td>${ob.username}</td>
				    ...
				    <td class="td-manage">
					<#if ob.status == '已上线'>
						<a style="text-decoration:none"  onclick="application.changeStatus(this,'未上线',${ob.id},'${ctx}/problem/changeStatus')"><i class="Hui-iconfont">&#xe6de;</i></a> 
					<#else>
						<a style="text-decoration:none"  onclick="application.changeStatus(this,'已上线',${ob.id},'${ctx}/problem/changeStatus')"><i class="Hui-iconfont">&#xe603;</i></a> 
					</#if>
						<a style="text-decoration:none" class="ml-5" onclick="application.locationToUrl('购房知识编辑','${ctx}//problem/editPage?id=${ob.id}')" href="javascript:;" title="编辑"><i class="Hui-iconfont">&#xe6df;</i></a> 
...
<script type="text/javascript">
	...
	function change(result){
		...
		for(var i = 0 ; i<result.data.length ; i++){
			...
			img="<img src='${ctx}"+result.data[i].imgurl+"' style='width:200px;height:70px;'/>";
		    if(result.data[i].status=="已上线"){
		     status = '<td class="td-status"><span class="label label-success radius">已上线</span></td>';
		     btn= "<a style='text-decoration:none' onClick=\"application.changeStatus(this,'未上线',"+result.data[i].id+",'${ctx}/problem/changeStatus')\" href='javascript:;' title='上线'><i class='Hui-iconfont'>&#xe6de;</i></a>";
            }else{
              btn= "<a style='text-decoration:none' onClick=\"application.changeStatus(this,'已上线',"+result.data[i].id+",'${ctx}/problem/changeStatus')\" href='javascript:;' title='下线'><i class='Hui-iconfont'>&#xe603;</i></a>";
			...
			$("#pagetbody").append("<tr class='text-c'>" +
					  ...
					  "<td class='td-manage'>" +
						  btn +
						  "<a style='text-decoration:none' class='ml-5' onclick=\"application.locationToUrl('购房知识编辑','${ctx}/problem/editPage?id="+result.data[i].id+"')\"  title='编辑'><i class='Hui-iconfont'>&#xe6df;</i></a> " +
						  "<a style='text-decoration:none' class='ml-5' onclick=\"application.deleteOne(this,"+result.data[i].id+",'${ctx}\/problem\/delete')\" href='javascript:;' title='删除'><i class='Hui-iconfont'>&#xe6e2;</i></a>" +
					  "</td></tr>");
